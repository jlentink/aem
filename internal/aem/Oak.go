...
func oakVersionForAEM(v, d string) string {
	switch v {
	case "6.0":
		return "1.0.42"
	case "6.1":
...
/*
func DownloadOak(aem, version, defaultVersion string) (string, error) {
	p := project.NewStructure()

	if len(aem) > 0 {
		version = OakVersionForAEM(aem, defaultVersion)
	}

	if len(version) == 0 {
		return ``, errors.New("no oak version selected. Use --aem or --oak to define version")
	}

	path, err := p.createBinDir()

	oakJar := path + fmt.Sprintf(oakJar, version)
	if !p.Exists(oakJar) {
